import {
  ...
  // Tables pour les notifications Push
  pushSubscriptions,
  type User,
  type UpsertUser,
  type Project,
...
export interface IStorage {
  ...
  updateLiveShowBattleInvestment(
    id: string,
    updates: Partial<LiveShowBattleInvestment>,
  ): Promise<LiveShowBattleInvestment>
  ...
  getUsersByRole(role: "admin" | "investor" | "creator" | "invested_reader" | "infoporteur"): Promise<User[]>
  updateUserProfiles(
    userId: string,
    profileTypes: ("investor" | "invested_reader" | "creator" | "admin" | "infoporteur")[],
  ): Promise<User>
  addUserProfile(
    userId: string,
    profileType: "investor" | "invested_reader" | "creator" | "admin" | "infoporteur",
  ): Promise<User>
  removeUserProfile(
    userId: string,
    profileType: "investor" | "invested_reader" | "creator" | "admin" | "infoporteur",
  ...
  createSocialPost(post: InsertSocialPost, tx?: any): Promise<SocialPost>
  ...
  getAllSocialPosts(limit?: number, offset?: number, userId?: string): Promise<SocialPost[]>

  // Social comments operations
  createSocialComment(comment: InsertSocialComment, tx?: any): Promise<SocialComment>
  ...
  createSocialLike(like: InsertSocialLike, tx?: any): Promise<SocialLike>
  ...
  createVisuPointsTransaction(transaction: InsertVisuPointsTransaction, tx?: any): Promise<VisuPointsTransaction>
  ...
  updateAdPhoto(
    id: string,
    updates: Pick<Partial<AdPhotos>, "alt" | "storageKey" | "width" | "height" | "bytes" | "contentType" | "sha256">,
  ): Promise<AdPhotos>
  ...
  upsertProjectMonthlyRanking(data: InsertProjectMonthlyRanking): Promise<ProjectMonthlyRanking>
  getMonthlyProjectRankings(monthYear: string, limit?: number): Promise<ProjectMonthlyRanking[]>
  getProjectRankingHistory(projectId: string, limit?: number): Promise<ProjectMonthlyRanking[]>
  getAvailableRankingMonths(limit?: number): Promise<string[]>
  getTopPerformersAllTime(limit?: number): Promise<any[]>
  getProjectsComparison(projectIds: string[], monthYear: string): Promise<ProjectMonthlyRanking[]>
...
export class DatabaseStorage implements IStorage {
  ...
  async getProjects(limit = 50, offset = 0, category?: string): Promise<Project[]> {
    ...
    return await baseQuery.orderBy(desc(projects.createdAt)).limit(limit).offset(offset)
  }
  ...
  async getActiveLiveShows(): Promise<LiveShow[]> {
    return await db.select().from(liveShows).where(eq(liveShows.isActive, true)).orderBy(desc(liveShows.createdAt))
  }
  ...
  async updateLiveShow(id: string, updates: Partial<LiveShow>): Promise<LiveShow> {
    const [updated] = await db.update(liveShows).set(updates).where(eq(liveShows.id, id)).returning()
    return updated
  }

  // Live Show Weekly Edition operations
  async createLiveShowEdition(data: InsertLiveShowEdition): Promise<LiveShowEdition> {
    const [edition] = await db.insert(liveShowEditions).values(data).returning()
    return edition
  }

  async getLiveShowEdition(id: string): Promise<LiveShowEdition | undefined> {
    const [edition] = await db.select().from(liveShowEditions).where(eq(liveShowEditions.id, id)).limit(1)
    return edition
  }

  async updateLiveShowEdition(id: string, updates: Partial<LiveShowEdition>): Promise<LiveShowEdition> {
    const [updated] = await db.update(liveShowEditions).set(updates).where(eq(liveShowEditions.id, id)).returning()
    return updated
  }
  ...
  async getCurrentWeekEdition(weekNumber: number, year: number): Promise<LiveShowEdition | undefined> {
    const [edition] = await db
      ...
      .where(and(eq(liveShowEditions.weekNumber, weekNumber), eq(liveShowEditions.year, year)))
      .limit(1)
    return edition
  }
  ...
  async createLiveShowCandidate(data: InsertLiveShowCandidate): Promise<LiveShowCandidate> {
    const [candidate] = await db.insert(liveShowCandidates).values(data).returning()
    return candidate
  }

  async getLiveShowCandidate(id: string): Promise<LiveShowCandidate | undefined> {
    const [candidate] = await db.select().from(liveShowCandidates).where(eq(liveShowCandidates.id, id)).limit(1)
    return candidate
  }

  async updateLiveShowCandidate(id: string, updates: Partial<LiveShowCandidate>): Promise<LiveShowCandidate> {
    const [updated] = await db.update(liveShowCandidates).set(updates).where(eq(liveShowCandidates.id, id)).returning()
    return updated
  }
  ...
  async getLiveShowCandidates(editionId: string, status?: string): Promise<LiveShowCandidate[]> {
    if (status) {
      return await db
        ...
        .where(and(eq(liveShowCandidates.editionId, editionId), eq(liveShowCandidates.status, status as any)))
  ...
  async createLiveShowCommunityVote(data: InsertLiveShowCommunityVote): Promise<LiveShowCommunityVote> {
    const [vote] = await db.insert(liveShowCommunityVotes).values(data).returning()
    return vote
  }

  async getCommunityVotesForCandidate(candidateId: string): Promise<LiveShowCommunityVote[]> {
    return await db.select().from(liveShowCommunityVotes).where(eq(liveShowCommunityVotes.candidateId, candidateId))
  }

  // Live Show Battle Investments operations
  async createLiveShowBattleInvestment(data: InsertLiveShowBattleInvestment): Promise<LiveShowBattleInvestment> {
    const [investment] = await db.insert(liveShowBattleInvestments).values(data).returning()
    return investment
  }
  ...
  async updateLiveShowBattleInvestment(
    id: string,
    updates: Partial<LiveShowBattleInvestment>,
  ): Promise<LiveShowBattleInvestment> {
    const [updated] = await db
  ...
  async createLiveShowAd(data: InsertLiveShowAd): Promise<LiveShowAd> {
    const [ad] = await db.insert(liveShowAds).values(data).returning()
    return ad
  }
  ...
  async updateLiveShowAd(id: string, updates: Partial<LiveShowAd>): Promise<LiveShowAd> {
    const [updated] = await db.update(liveShowAds).set(updates).where(eq(liveShowAds.id, id)).returning()
    return updated
  }
  ...
  async getComplianceReports(limit = 50): Promise<ComplianceReport[]> {
    return await db.select().from(complianceReports).orderBy(desc(complianceReports.createdAt)).limit(limit)
  }
  ...
  async getVideoDeposit(id: string): Promise<VideoDeposit | undefined> {
    const [deposit] = await db.select().from(videoDeposits).where(eq(videoDeposits.id, id))
    return deposit
  }

  async getVideoDepositByPaymentIntent(paymentIntentId: string): Promise<VideoDeposit | undefined> {
    const [deposit] = await db.select().from(videoDeposits).where(eq(videoDeposits.paymentIntentId, paymentIntentId))
    return deposit
  }
  ...
  async getVideoToken(token: string): Promise<VideoToken | undefined> {
    const [videoToken] = await db.select().from(videoTokens).where(eq(videoTokens.token, token))
    return videoToken
  }
  ...
  async updateVideoToken(id: string, updates: Partial<VideoToken>): Promise<VideoToken> {
    const [updatedToken] = await db.update(videoTokens).set(updates).where(eq(videoTokens.id, id)).returning()
    return updatedToken
  }

  async revokeVideoTokens(videoDepositId: string): Promise<void> {
    await db.update(videoTokens).set({ isRevoked: true }).where(eq(videoTokens.videoDepositId, videoDepositId))
  }
  ...
  async updateSocialComment(id: string, updates: Partial<SocialComment>): Promise<SocialComment> {
    const [updatedComment] = await db.update(socialComments).set(updates).where(eq(socialComments.id, id)).returning()
  ...
  async removeSocialLike(userId: string, postId?: string, commentId?: string): Promise<void> {
    if (postId) {
      await db.delete(socialLikes).where(and(eq(socialLikes.userId, userId), eq(socialLikes.postId, postId)))
    } else if (commentId) {
      await db.delete(socialLikes).where(and(eq(socialLikes.userId, userId), eq(socialLikes.commentId, commentId)))
    }
  }
  ...
  // VisuPoints transactions operations
  async createVisuPointsTransaction(
    transaction: InsertVisuPointsTransaction,
    tx?: any,
  ...
  async getPendingPurgeJobs(): Promise<PurgeJob[]> {
    return await db.select().from(purgeJobs).where(eq(purgeJobs.status, "pending")).orderBy(asc(purgeJobs.targetDate))
  }
  ...
  async updatePurgeJob(id: string, updates: Partial<PurgeJob>): Promise<PurgeJob> {
    const [updatedJob] = await db.update(purgeJobs).set(updates).where(eq(purgeJobs.id, id)).returning()
    return updatedJob
  }
  ...
  async getPaymentReceiptByTransaction(transactionId: string): Promise<PaymentReceipt | undefined> {
    const [receipt] = await db.select().from(paymentReceipts).where(eq(paymentReceipts.transactionId, transactionId))
    return receipt
  }
  ...
  async getVideoCategory(category: string): Promise<VideoCategory | undefined> {
    const [videoCategory] = await db.select().from(videoCategories).where(eq(videoCategories.name, category))
    return videoCategory
  }

  async getVideoCategoryById(id: string): Promise<VideoCategory | undefined> {
    const [videoCategory] = await db.select().from(videoCategories).where(eq(videoCategories.id, id))
    return videoCategory
  }

  async getAllVideoCategories(): Promise<VideoCategory[]> {
    return await db.select().from(videoCategories).orderBy(asc(videoCategories.name))
  }
  ...
  async getCategoryActiveVideos(category: string): Promise<number> {
    ...
    const result = await db
      ...
      .where(and(eq(projects.category, category), eq(videoDeposits.status, "active")))
  ...
  async getContentReports(limit = 50, offset = 0): Promise<ContentReport[]> {
    return await db.select().from(contentReports).orderBy(desc(contentReports.createdAt)).limit(limit).offset(offset)
  }
  ...
  async getContentReportsByContent(contentType: string, contentId: string): Promise<ContentReport[]> {
    return await db
      .select()
      .from(contentReports)
      .where(and(eq(contentReports.contentType, contentType as any), eq(contentReports.contentId, contentId)))
      .orderBy(desc(contentReports.createdAt))
  }
  ...
  async updateReferral(id: string, updates: Partial<Referral>): Promise<Referral> {
    const [updatedReferral] = await db.update(referrals).set(updates).where(eq(referrals.id, id)).returning()
  ...
  async getUserReferralLimit(userId: string, monthYear: string): Promise<ReferralLimit | undefined> {
    const [limit] = await db
      .select()
      .from(referralLimits)
      .where(and(eq(referralLimits.userId, userId), eq(referralLimits.monthYear, monthYear)))
    return limit
  }
  ...
  async updateReferralLimit(
    userId: string,
    monthYear: string,
    updates: Partial<ReferralLimit>,
  ): Promise<ReferralLimit> {
  ...
  async getVisitorOfMonth(userId: string, monthYear: string): Promise<VisitorOfMonth | undefined> {
    const [visitor] = await db
      ...
      .where(and(eq(visitorsOfMonth.userId, userId), eq(visitorsOfMonth.monthYear, monthYear)))
    return visitor
  }
  ...
  async updateVisitorOfMonth(
    userId: string,
    monthYear: string,
    updates: Partial<VisitorOfMonth>,
  ): Promise<VisitorOfMonth> {
  ...
  async getArticlesByIds(ids: string[]): Promise<Article[]> {
    ...
    return await db.select().from(articles).where(sql`${articles.id} = ANY(${ids})`)
  }
  ...
  async getArticles(limit = 20, offset = 0, category?: string): Promise<Article[]> {
    ...
    return await query.orderBy(desc(articles.createdAt)).limit(limit).offset(offset)
  }
  ...
  async getTop10ByDate(date: Date): Promise<Top10Infoporteurs[]> {
    ...
    return await db
      ...
      .where(and(gte(top10Infoporteurs.rankingDate, startOfDay), lte(top10Infoporteurs.rankingDate, endOfDay)))
      .orderBy(asc(top10Infoporteurs.rank))
  }

  async updateTop10Infoporteur(id: string, updates: any): Promise<Top10Infoporteurs> {
    const [result] = await db.update(top10Infoporteurs).set(updates).where(eq(top10Infoporteurs.id, id)).returning()
    return result
  }
  ...
  async getTop10WinnersByDate(date: Date): Promise<Top10Winners[]> {
    ...
    return await db
      .select()
      .from(top10Winners)
      .where(and(gte(top10Winners.rankingDate, startOfDay), lte(top10Winners.rankingDate, endOfDay)))
      .orderBy(desc(top10Winners.totalInvestedEUR))
  }

  async updateTop10Winner(id: string, updates: any): Promise<Top10Winners> {
    const [result] = await db.update(top10Winners).set(updates).where(eq(top10Winners.id, id)).returning()
    return result
  }
  ...
  async updateWeeklyStreak(userId: string, updates: any): Promise<WeeklyStreaks> {
    const [result] = await db.update(weeklyStreaks).set(updates).where(eq(weeklyStreaks.userId, userId)).returning()
    return result
  }
  ...
  async getArticleSaleDaily(articleId: string, saleDate: string): Promise<ArticleSalesDaily | undefined> {
    ...
    const [result] = await db
      ...
      .where(
        and(eq(articleSalesDaily.articleId, articleId), sql`DATE(${articleSalesDaily.salesDate}) = DATE(${dateObj})`),
      )
    return result
  }
  ...
  async getStripeTransferById(id: string): Promise<StripeTransfer | undefined> {
    const [result] = await db.select().from(stripeTransfers).where(eq(stripeTransfers.id, id)).limit(1)
    return result
  }
  ...
  async getProjectRenewal(id: string): Promise<ProjectRenewal | undefined> {
    const [renewal] = await db.select().from(projectRenewals).where(eq(projectRenewals.id, id))
    return renewal
  }
  ...
  async getActiveRenewals(categoryId: string): Promise<ProjectRenewal[]> {
    return await db
      .select()
      .from(projectRenewals)
      .where(and(eq(projectRenewals.categoryId, categoryId), eq(projectRenewals.status, "active")))
  ...
  async getProjectQueue(categoryId: string): Promise<ProjectQueue[]> {
    return await db
      .select()
      .from(projectQueue)
      .where(and(eq(projectQueue.categoryId, categoryId), eq(projectQueue.isActive, true)))
  ...
  async removeProjectFromQueue(id: string): Promise<void> {
    await db.update(projectQueue).set({ isActive: false, updatedAt: new Date() }).where(eq(projectQueue.id, id))
  }
  ...
  async updateQueuePositions(categoryId: string): Promise<void> {
    ...
    const activeItems = await db
      ...
      .from(projectQueue)
      .where(and(eq(projectQueue.categoryId, categoryId), eq(projectQueue.isActive, true)))
      .orderBy(eq(projectQueue.submittedAt))
  ...
  async createProjectReplacement(replacement: InsertProjectReplacement): Promise<ProjectReplacement> {
    ...
    const replacementWithKey = {
      ...replacement,
      idempotencyKey:
        replacement.idempotencyKey || `${replacement.categoryId}-${replacement.replacedRank}-${Date.now()}`,
    }
  ...
  async getAgentDecisions(agentType?: string, status?: string, limit = 50): Promise<AgentDecision[]> {
    ...
    return await query.orderBy(desc(agentDecisions.createdAt)).limit(limit)
  }
  ...
  async updateAgentDecisionStatus(
    id: string,
    status: string,
    adminComment?: string,
    validatedBy?: string,
    ...
    const [updated] = await db.update(agentDecisions).set(updates).where(eq(agentDecisions.id, id)).returning()
  ...
  async createAuditLogEntry(entry: InsertAgentAuditLog): Promise<AgentAuditLog> {
    ...
    const previousEntry = await db.select().from(agentAuditLog).orderBy(desc(agentAuditLog.id)).limit(1)
  ...
  async getAuditLog(agentType?: string, action?: string, limit = 100): Promise<AgentAuditLog[]> {
    ...
    return await query.orderBy(desc(agentAuditLog.timestamp)).limit(limit)
  }
  ...
  async getAuditLogBySubject(subjectType: string, subjectId: string): Promise<AgentAuditLog[]> {
    return await db
      .select()
      .from(agentAuditLog)
      .where(and(eq(agentAuditLog.subjectType, subjectType), eq(agentAuditLog.subjectId, subjectId)))
      .orderBy(desc(agentAuditLog.timestamp))
  }

  async validateAuditChain(): Promise<boolean> {
    // Validate the hash chain integrity
    const entries = await db.select().from(agentAuditLog).orderBy(agentAuditLog.id)
  ...
  async getLedgerEntries(referenceType?: string, recipientId?: string, limit = 100): Promise<FinancialLedger[]> {
    ...
    return await query.orderBy(desc(financialLedger.createdAt)).limit(limit)
  }
  ...
  async updateLedgerStatus(id: string, status: string, processedAt?: Date): Promise<FinancialLedger> {
    ...
    const [updated] = await db.update(financialLedger).set(updates).where(eq(financialLedger.id, id)).returning()
  ...
  async reconcileLedgerWithStripe(): Promise<{ divergences: number; total: number }> {
    ...
    const entries = await db
      .select()
      .from(financialLedger)
      .where(and(eq(financialLedger.status, "completed"), isNotNull(financialLedger.stripePaymentIntentId)))
  ...
  async getActivePayoutRecipe(ruleType: string): Promise<PayoutRecipe | undefined> {
    const [recipe] = await db
      .select()
      .from(payoutRecipes)
      .where(and(eq(payoutRecipes.ruleType, ruleType), eq(payoutRecipes.isActive, true)))
  ...
  async activatePayoutRecipe(version: string): Promise<PayoutRecipe> {
    ...
    const [recipe] = await db.select().from(payoutRecipes).where(eq(payoutRecipes.version, version))
    ...
    await db.update(payoutRecipes).set({ isActive: false }).where(eq(payoutRecipes.ruleType, recipe.ruleType))
  ...
  async getAgentParameter(key: string): Promise<AgentParameter | undefined> {
    const [param] = await db.select().from(agentParameters).where(eq(agentParameters.parameterKey, key))
    return param
  }
  ...
  async getBookCategoryByName(name: string): Promise<BookCategory | undefined> {
    const [category] = await db.select().from(bookCategories).where(eq(bookCategories.name, name))
    return category
  }

  async getAllBookCategories(): Promise<BookCategory[]> {
    return await db.select().from(bookCategories).orderBy(asc(bookCategories.createdAt))
  }

  async updateBookCategory(id: string, updates: Partial<BookCategory>): Promise<BookCategory> {
    const [updated] = await db.update(bookCategories).set(updates).where(eq(bookCategories.id, id)).returning()
    return updated
  }
  ...
  async getBooksByAuthor(authorId: string): Promise<Book[]> {
    return await db.select().from(books).where(eq(books.authorId, authorId)).orderBy(desc(books.createdAt))
  }

  async getBooksByCategoryId(categoryId: string): Promise<Book[]> {
    return await db.select().from(books).where(eq(books.categoryId, categoryId)).orderBy(desc(books.createdAt))
  }
  ...
  async getPendingBooks(): Promise<Book[]> {
    return await db.select().from(books).where(eq(books.status, "pending")).orderBy(asc(books.createdAt))
  }

  async getActiveBooks(categoryId?: string): Promise<Book[]> {
    let query = db.select().from(books).where(eq(books.status, "active"))
  ...
  async getBookPurchase(id: string): Promise<BookPurchase | undefined> {
    const [purchase] = await db.select().from(bookPurchases).where(eq(bookPurchases.id, id))
    return purchase
  }
  ...
  async updateBookPurchase(id: string, updates: Partial<BookPurchase>): Promise<BookPurchase> {
    const [updated] = await db.update(bookPurchases).set(updates).where(eq(bookPurchases.id, id)).returning()
    return updated
  }
  ...
  async updateDownloadToken(token: string, updates: Partial<DownloadToken>): Promise<DownloadToken> {
    const [updated] = await db.update(downloadTokens).set(updates).where(eq(downloadTokens.token, token)).returning()
    return updated
  }
  ...
  async createPetiteAnnonce(annonce: InsertPetitesAnnonces): Promise<PetitesAnnonces> {
    const [newAnnonce] = await db
      .insert(petitesAnnonces)
      .values({
        ...annonce,
        id: nanoid(),
        viewCount: 0,
  ...
  async getPetiteAnnonce(id: string): Promise<PetitesAnnonces | undefined> {
    const [annonce] = await db.select().from(petitesAnnonces).where(eq(petitesAnnonces.id, id))
    return annonce
  }

  async getPetitesAnnonces(
    params: {
      category?: string
      location?: string
      authorId?: string
    ...
    return await db
      ...
      .where(
        and(
          ...
          or(eq(petitesAnnonces.status, "approved"), eq(petitesAnnonces.status, "pending")),
        ),
      )
  ...
  async getExpiredAnnonces(): Promise<PetitesAnnonces[]> {
    ...
    return await db
      .select()
      .from(petitesAnnonces)
      .where(and(lte(petitesAnnonces.expiresAt, now), eq(petitesAnnonces.status, "approved")))
  }
  ...
  async rejectAnnonce(annonceId: string, moderatorId: string, reason: string, sanction?: string): Promise<void> {
    await db.transaction(async (tx) => {
      ...
      const [annonce] = await tx.select().from(petitesAnnonces).where(eq(petitesAnnonces.id, annonceId))
    ...
    return await db
      ...
      .where(
        and(
          ...
          or(isNotNull(annoncesSanctions.expiresAt), gte(annoncesSanctions.expiresAt, new Date())),
        ),
      )
  ...
  async getUserEscrowTransactions(userId: string, role?: "buyer" | "seller"): Promise<EscrowTransactions[]> {
    ...
    if (role === "buyer") {
      ...
      conditions.push(or(eq(escrowTransactions.buyerId, userId), eq(escrowTransactions.sellerId, userId)))
    }
  ...
  async updateEscrowStatus(
    id: string,
    status: string,
    updates: Partial<EscrowTransactions> = {},
  ): Promise<EscrowTransactions> {
  ...
  async getAdPhoto(id: string): Promise<AdPhotos | undefined> {
    const [photo] = await db.select().from(adPhotos).where(eq(adPhotos.id, id))
    return photo
  }

  async getAdPhotos(adId: string): Promise<AdPhotos[]> {
    return await db.select().from(adPhotos).where(eq(adPhotos.adId, adId)).orderBy(adPhotos.idx)
  }
  ...
  async updateAdPhoto(
    ...
    const [updatedPhoto] = await db.update(adPhotos).set(updates).where(eq(adPhotos.id, id)).returning()
  ...
  async deleteAdPhoto(id: string): Promise<void> {
    await db.delete(adPhotos).where(eq(adPhotos.id, id))
  }
  ...
  async setCoverPhoto(adId: string, photoId: string): Promise<void> {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.update(adPhotos).set({ isCover: false }).where(eq(adPhotos.adId, adId))

      // Set new cover photo - unique constraint ensures atomicity
      await tx.update(adPhotos).set({ isCover: true }).where(eq(adPhotos.id, photoId))
    })
  }
  ...
  async getPendingPhotoModeration(): Promise<AdPhotos[]> {
    return await db.select().from(adPhotos).where(eq(adPhotos.moderationStatus, "pending")).orderBy(adPhotos.createdAt)
  }
  ...
  async updateUser2FAStatus(userId: string, status: string): Promise<void> {
    await db.update(user2FA).set({ status }).where(eq(user2FA.userId, userId))
  }

  async updateUser2FALastUsed(userId: string): Promise<void> {
    await db.update(user2FA).set({ lastUsedAt: new Date() }).where(eq(user2FA.userId, userId))
  }
  ...
  async getSecurityAuditLogs(limit = 100, offset = 0, actorId?: string): Promise<SecurityAuditLog[]> {
    ...
    return await baseQuery.orderBy(desc(securityAuditLog.timestamp)).limit(limit).offset(offset)
  }
  ...
  async markStripeEventProcessed(id: string): Promise<void> {
    await db.update(stripeEvents).set({ processed: true, updatedAt: new Date() }).where(eq(stripeEvents.id, id))
  }
  ...
  async getEbooks(status?: string, limit = 50): Promise<Ebook[]> {
    if (status) {
      return await db
        .select()
        .from(ebooks)
        .where(eq(ebooks.status, status as any))
        .limit(limit)
    }
  ...
  async getEbookLicenseByUserAndBook(
    userId: string,
    ebookId: string,
    orderId: string,
  ): Promise<EbookLicense | undefined> {
    const [result] = await db
      .select()
      .from(ebookLicenses)
      .where(
        and(eq(ebookLicenses.userId, userId), eq(ebookLicenses.ebookId, ebookId), eq(ebookLicenses.orderId, orderId)),
      )
    return result
  }
  ...
  async updateEbookDownloadAttempt(id: string, updates: Partial<EbookDownloadAttempt>): Promise<EbookDownloadAttempt> {
    const [result] = await db
      ...
      .set(updates)
      .where(eq(ebookDownloadAttempts.id, id))
      .returning()
    return result
  }
  ...
  async expireOldEbookDownloadAttempts(): Promise<number> {
    const result = await db
      .update(ebookDownloadAttempts)
      .set({ status: "expired" })
      .where(and(eq(ebookDownloadAttempts.status, "pending"), lt(ebookDownloadAttempts.expiresAt, new Date())))
      .returning()
  ...
  async upsertProjectMonthlyRanking(data: InsertProjectMonthlyRanking): Promise<ProjectMonthlyRanking> {
    const [existing] = await db
      ...
      .where(
        and(eq(projectMonthlyRankings.projectId, data.projectId), eq(projectMonthlyRankings.monthYear, data.monthYear)),
      )
    ...
    const [created] = await db.insert(projectMonthlyRankings).values(data).returning()
    return created
  }
  ...
  async savePushSubscription(
    ...
    await db
      ...
      .values({
        ...
        userAgent: "unknown", // Can be enhanced to capture actual user agent
      })
      .onConflictDoUpdate({
        target: pushSubscriptions.userId, // Assuming userId should be unique for subscription
        set: {
          endpoint: subscription.endpoint,
          p256dh: subscription.keys.p256dh,
          auth: subscription.keys.auth,
  ...
  async getAllPushSubscribedUsers(): Promise<string[]> {
